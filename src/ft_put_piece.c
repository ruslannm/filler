...
/*   Updated: 2020/03/05 18:00:25 by rgero            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_put_piece_algoritm(t_map **map)
{
	int h;
	int w;
...
int		ft_put_piece(t_map **map)
{
	...
	distance = ft_get_distance_opposite(*map);
	if (0 == distance)
		(*map)->opposite_reach = SHOTS + 1;
	else if (distance < (*map)->piece_last_distance_opposite)
		(*map)->piece_last_distance_opposite = distance;
	else
